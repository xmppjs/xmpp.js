...
function Connection(opts) {
    ...
    if (opts && opts.setup)
        opts.setup(this)
    this.connect({socket: opts && opts.socket})
    if (opts && typeof opts.reconnect === 'boolean')
...
Connection.prototype._setupSocket = function (params) {
    var inject = reconnect(function (opts) {
        this.socket = null
        if (opts && typeof opts.socket === 'function') {
            /* lazy evaluation
    ...
    this.on('connection', this.startParser.bind(this))
    this.on('disconnect', function (err) {
        this.stopParser()
        // stop reconnecting when error emitted
        if (err) this.reconnect = false
    })
