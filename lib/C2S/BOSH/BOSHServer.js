...
function BOSHServer(options) {
    ...
    var server = this.server = this.options.server || http.createServer()
     ...
     * see BOSHServer.prototype.close
     * we understand this is not a Node.js bug
     * but users will expect close on all C2S servers to work the same
    */
    var self = this
...
BOSHServer.prototype._createSession = function(req, res, bodyEl) {
    ...
    var self = this
    var session = new Socket({ req: req, res: res, bodyEl: bodyEl, nextRequestTimeout: this.nextRequestTimeout})
