...
Connection.prototype.send = function(stanza) {
    ...
    if (stanza.root) {
        ...
        if(typeof this.socket.serialize === "undefined") {
            this.socket.serialize = function(el, clbk) {
                el.write(function(s) { 
                    clbk(s); 
                });
            }
        }
        this.socket.serialize(el, function(s) {
            self.socket.write(s); 
        });
        return el;
    } else {
...
Connection.prototype.setSecure = function(credentials, isServer) {
    ...
    ct.encrypted.on('error', function(e) {
	    console.error({encrypted: true, ctEError:e});
    });
    ct.on('error', function(e) {
	    console.error({encrypted: false, ctError:e});
    });
