...
Connection.prototype.setSecure = function(credentials, isServer) {
    var self = this;
    /* Rescue some TLS handshake start from the parser */
    var tlsData, parseBuffer = this.parser.parseBuffer;
    if (parseBuffer) {
	for(var i = 0; i < parseBuffer.length; i++) {
	    /* 0x16 starts a TLS handshake. It is also invalid UTF-8 XML. */
	    if (parseBuffer[i] === 0x16) {
		/* Control char found? TLS probably starts here. */
		tlsData = parseBuffer.slice(i);
		break;
	    }
	}
    }
    ...
    var ct = starttls(this.socket, credentials || this.credentials, isServer, function() {
			  console.log('hand shaked');
        self.startParser();
        self.startStream();
    });
    ct.encrypted.on('error', function(e) {
	console.error({ctEError:e});
    });
    ct.on('error', function(e) {
	console.error({ctError:e});
    });
    /* Rescued handshake start: */
    if (tlsData)
	ct.encrypted.write(tlsData);
