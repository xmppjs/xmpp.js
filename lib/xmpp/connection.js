...
Connection.prototype.setSecure = function(self, credentials, isServer) {
//    var self = this;

    // Remove old event listeners
    self.socket.removeAllListeners('data');
    // retain socket 'end' listeners because ssl layer doesn't support it
    self.socket.removeAllListeners('drain');
    self.socket.removeAllListeners('close');
    // remove idle_timeout
    if (self.socket.clearTimer)
	self.socket.clearTimer();

    self.stopParser();
    var ct = starttls(self.socket, credentials || self.credentials, isServer, function() {
	self.isSecure = true;
	self.startParser();
	if (!isServer)
