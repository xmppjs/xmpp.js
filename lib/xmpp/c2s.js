...
function C2SStream(opts) {
	...
	console.log("C2SStream attrs <<", streamAttrs);
	console.log("streamAttrs",self.connection.streamAttrs,streamAttrs);
	self.domain = streamAttrs.to;
...
C2SStream.prototype.sendFeatures = function() {
    ...
    if (!this.authenticated) {
        // TLS
        if (this.server && this.server.options.tls && !this.socket.encrypted) {
