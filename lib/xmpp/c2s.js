 ...
 */
C2S.prototype.supports = function(mod) {
    // var m = new mod();
    if(this.mods.hasOwnProperty(mod.name)) {
        // Raise error!
        console.log("MOD CONFLICT, NAME ALREADY EXISTS: " + mod.name);
    }
    else {
...
function C2SClient(socket, c2s) {
    ...
    this.credentials = { key: fs.readFileSync(c2s.options.tls.keyPath, 'ascii'),
			 cert: fs.readFileSync(c2s.options.tls.certPath, 'ascii')
		       };
...
C2SClient.prototype.onRawStanza = function(stanza) {
    ...
    if(stanza.is('starttls', NS_XMPP_TLS)) {
        ...
        // First, let's send this stanza to all the mods... just in case any of them needs it. Should we do that in the form of events?
        for(mod in this.c2s.mods) {
            this.c2s.mods[mod].onRawStanza(stanza, this);
        }
