...
C2SClient.prototype.onRawStanza = function(stanza) {
    ...
    else if (stanza.is('iq') && stanza.attrs.type == 'set' && (bind = stanza.getChild('bind', 'urn:ietf:params:xml:ns:xmpp-bind'))) {
        this.onBind(stanza);
    }
    else if (stanza.is('iq') &&  stanza.attrs.type == 'set' && (session = stanza.getChild('session', 'urn:ietf:params:xml:ns:xmpp-session'))) {
        this.onSession(stanza);
    }
    else if (stanza.attrs.to && stanza.attrs.to != "" && stanza.attrs.to != this.c2s.options.domain && stanza.attrs.to != this.jid.bare().toString()) {
...
C2SClient.prototype.onBind = function(stanza) {
    ...
    if(!self.c2s.sessions.hasOwnProperty(self.jid.bare().toString()))
    self.c2s.sessions[self.jid.bare().toString()] = {}; 
    self.c2s.sessions[self.jid.bare().toString()][self.jid.resource] = self;
