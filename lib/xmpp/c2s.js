...
C2SStream.prototype.sendFeatures = function() {
    ...
    if (!this.authenticated) {
        ...
        var mechanismsEl = features.c("mechanisms", {
            xmlns: NS_XMPP_SASL
        });
...
C2SStream.prototype.authenticate = function(user) {
    var self = this;
    var jid = new JID(user.username, this.serverdomain ? this.serverdomain.toString() : "");
    user.jid = jid;
    user.client = this;
...
C2SStream.prototype.onAuth = function(stanza) {
    ...
    if (matchingMechs[0]) {
        ...
        this.mechanism.extractSasl(decode64(stanza.getText()), function(user){
