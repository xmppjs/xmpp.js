...
C2SStream.prototype.sendFeatures = function() {
    ...
    if (!this.authenticated) {
        if (this.server) {
            // TLS
            if (this.server.options.tls && !this.connection.socket.encrypted) {
...
C2SStream.prototype.onStanza = function(stanza) {
    ...
    if (stanza.is('starttls', NS_XMPP_TLS)) {
        ...
        this.setSecure(this.connection, this.server.credentials, true);
...
C2SStream.prototype.authenticate = function(username, password) {
    var self = this;
    var jid = new JID(username, this.domains ? this.domains.toString() : "");
