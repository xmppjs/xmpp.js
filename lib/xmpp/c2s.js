...
C2SServer.prototype.acceptConnection = function(socket, options) {
	...
	var userJid = client.jid.bare().toString();
	if (!self.sessions.hasOwnProperty(userJid))
	    self.sessions[userJid] = [];
	// TODO: does session exist already? kill it!
	/* Add session entry */
	self.sessions[userJid].push({
	    resource: client.jid.resource,
	    client: client,
	    presence: null
	});
	client.addListener('disconnect', function() {
	    ...
	    self.sessions[userJid] =
		self.sessions[userJid].filter(function(session) {
		    return !(session.client === client);
		});
	    if (self.sessions[userJid].length < 1)
		delete self.sessions[userJid];
	});
    });
};
...
C2SStream.prototype.onRawStanza = function(stanza) {
    ...
    if (stanza.is('starttls', NS_XMPP_TLS)) {
        ...
        if (this.authenticated) {
            this.emit('inStanza', stanza); // We make the distinction between inStanza (coming from the C2SStream), and outStanza (going to the C2SStream)
        }
    }
};
