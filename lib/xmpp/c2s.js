...
function C2SClient(socket, c2s) {
    ...
    this.addListener('rawStanza', function(stanza) {
        ...
        if(self.jid) {
            stanza.attrs.from = self.jid.toString();
        }
        ...
        else if (stanza.attrs.to && stanza.attrs.to != "" && stanza.attrs.to != self.c2s.options.domain && stanza.attrs.to != self.jid.bare().toString()) {
            // If the message is not sent to the server!
            self.c2s.router.send(stanza);
        }
        else {
