...
DigestMD5.prototype.manageAuth = function(stanza, server) {
    if (stanza.is('auth', NS_XMPP_SASL)) {
        ...
        var challenge = new ltx.Element('challenge', {
            xmlns: NS_XMPP_SASL
        ...
        if (responseValid) {
            ...
            self.failure('response not valid');
        }
    } else if (stanza.is('response', NS_XMPP_SASL)) {
        // here we are successfully authenticated and are able to call the callback
        // TODO we need to ensure the previous steps happened before, otherwise an authentication 
        // is possible without previous challenges
...
DigestMD5.prototype.loginFailed = function(server) {
    ...
    server.send(new ltx.Element('response', {
        xmlns: NS_XMPP_SASL
    }))
}
