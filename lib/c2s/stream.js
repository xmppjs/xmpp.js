...
C2SStream.prototype.onAuthStanza = function(stanza) {
    var bind
    if (stanza.is('auth', NS_XMPP_SASL) || stanza.is('response', NS_XMPP_SASL)) {
        this.onAuth(stanza)
 ...
 */
C2SStream.prototype.authenticate = function(error, user) {
    var self = this
    var jid = null;
    ...
    if (user.jid) {
        ...
        jid = new JID(user.username, this.serverdomain ?
...
C2SStream.prototype.onAuth = function(stanza) {
    ...
    if (this.mechanism) {
        this.mechanism.manageAuth(this, stanza, function(error, user) {
            user.saslmech = self.mechanism.name
            self.authenticate(error, user);
        });
    } else {
