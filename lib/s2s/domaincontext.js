...
var debug = require('debug')('domainctx')
...
DomainContext.prototype.establishS2SStream = function (destDomain) {
    debug('establishS2SStream')
    var self = this
    var credentials = this.router.credentials[this.domain]
    // Setup a new outgoing connection
    var outStream = new OutgoingServer(this.domain, destDomain, credentials)
...
DomainContext.prototype.startDialback = function (destDomain, outStream) {
    debug('startDialback')
    debug(JSON.stringify(outStream.streamId))
    ...
    var onResult = function (from, to, isValid) {
        debug('onResult: ' + from + to + isValid);
        if ((from !== destDomain) ||
            (to !== self.domain)) {
            // not for us
            return
...
DomainContext.prototype.verifyIncoming = function (fromDomain, inStream, dbKey) {
    debug('verifyIncoming')
    var self = this
    debug('streamid: ' + inStream.streamId);
